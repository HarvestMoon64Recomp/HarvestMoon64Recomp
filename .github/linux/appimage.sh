...
cp HarvestMoon64Recompiled AppDir/usr/bin/
cp -r assets/ AppDir/usr/bin/
cp recompcontrollerdb.txt AppDir/usr/bin/
cp icons/512.png AppDir/HarvestMoon64Recompiled.png
cp .github/linux/HarvestMoon64Recompiled.desktop AppDir/

"./linuxdeploy-$LINUX_DEPLOY_ARCH.AppImage" --appimage-extract
mv squashfs-root/ deploy
./deploy/AppRun --appdir=AppDir/ -d AppDir/HarvestMoon64Recompiled.desktop -i AppDir/HarvestMoon64Recompiled.png -e AppDir/usr/bin/HarvestMoon64Recompiled --plugin gtk
...
echo '    APP_FOLDER_PATH=$APP_FOLDER_PATH ./HarvestMoon64Recompiled' >> AppDir/AppRun
echo 'else' >> AppDir/AppRun
echo '    cd "$this_dir"/usr/bin/' >> AppDir/AppRun
echo '    ./HarvestMoon64Recompiled' >> AppDir/AppRun
